...
class PDFPhilosophicalSplitter:
    ...
    def __init__(self):
        # 匹配大标题：中文数字 + 空格/全角空格 + 标题文字
        # 支持："三 无之以为用"、"三　无之以为用"、"三、无之以为用" 等变体
        self.title_pattern = re.compile(
            r"^\s*([一二三四五六七八九十百千]+)[\s　、]+(.+?)\s*$", re.MULTILINE
        )
        # 清理 PDF 提取的噪声
    ...
    def identify_titles(self, documents: List[Document]) -> List[Dict[str, Any]]:
        """
        在所有页面中识别大标题位置
        返回: [{"title": "三 无之以为用", "page": 45, "char_idx": 120, "full_text": "..."}, ...]
        """
        titles = []
            ...
            parents.append(
                {
                    ...
                    "parent_id": f"chapter_{title_info['chapter_number']}",
                    "metadata": {
...
def process_philosophy_pdf(pdf_path: str, source_name: str = "") -> Tuple[int, int]:
    ...
    for parent in parents:
        children = splitter.split_children(
            parent, chunk_size=280, overlap=40  # 约 2-3 个自然段
        )
